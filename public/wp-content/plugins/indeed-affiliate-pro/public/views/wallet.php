...
<div class="uap-profile-box-wrapper">
        <div class="uap-profile-box-content">
        	<div class="uap-row ">
            	<div class="uap-col-xs-12">
                   <div class="uap-account-detault-message">
                     <a href="<?php echo esc_url($data['add_new']);?>" class="uap-addd-to-wallet"><?php esc_html_e('Add New Wallet Credit', 'uap');?></a>
                  </div>
          		</div>
             </div>
        </div>
...
<?php if ($data['items']):?>
	...
	<?php foreach ($data['items'] as $k=>$v):?>
		<tr>
			...
			<td><i onClick="uapRemoveWalletItem('<?php echo esc_attr($v['type']);?>', '<?php echo esc_attr($v['code']);?>');" class="fa-uap fa-trash-uap"></i></td>
		</tr>
	<?php endforeach;?>
	</table>
    <?php else: ?>
