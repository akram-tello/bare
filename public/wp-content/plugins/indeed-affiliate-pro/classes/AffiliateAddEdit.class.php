	...
	class AffiliateAddEdit{
		...
		private function check_username(){
			...
			if ($this->type=='create'){
				...
				if ( !validate_username( sanitize_text_field($_REQUEST['user_login']) ) ) {
					$this->errors['user_login'] = $this->register_metas['uap_register_error_username_msg'];
				}
				if ( username_exists( sanitize_text_field($_REQUEST['user_login']) ) ) {
				...
				if ($captcha!==FALSE && $this->register_fields[$captcha][$this->display_type]){
					...
					if (isset($_REQUEST['g-recaptcha-response'])){
						...
						if ($secret){
							...
							$resp = $recaptcha->verify( uap_sanitize_textarea_array($_POST['g-recaptcha-response']), $_SERVER['REMOTE_ADDR']);
							if (!$resp->isSuccess()){
