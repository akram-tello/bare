					...
					<table class="wp-list-table widefat fixed tags uap-admin-tables">
						...
						<tbody class="ui-sortable uap-alternate">
							<?php foreach ($data['ranks'] as $arr) : ?>
								<tr onmouseover="uapDhSelector('#rank_<?php echo esc_attr($arr->id);?>', 1);" onmouseout="uapDhSelector('#rank_<?php echo esc_attr($arr->id);?>', 0);">
									<td>
										<?php echo esc_uap_content("<b>" . esc_attr($arr->label) . "</b>");?>
										<div id="rank_<?php echo esc_attr($arr->id);?>" class="uap-visibility-hidden">
											<a href="<?php echo esc_url($data['url-add_edit'] . '&id=' . $arr->id);?>"><?php esc_html_e('Edit', 'uap');?></a>
											|
											<span class="uap-js-delete-ranks uap-delete-span" data-id="<?php echo esc_attr($arr->id);?>" ><?php esc_html_e('Delete', 'uap');?></span>
										</div>
									</td>
									<td><?php
										if ($arr->amount_type){
											if (!empty($this->amount_type_list[$arr->amount_type])){
												if ('%'==$this->amount_type_list[$arr->amount_type]){
													echo esc_html($arr->amount_value . ' ' . $this->amount_type_list[$arr->amount_type]);
												} else {
													echo uap_format_price_and_currency($this->amount_type_list[$arr->amount_type], $arr->amount_value);
												}
											} else {
												echo esc_html($arr->amount_value);
											}
										}
										?></td>
									<td><?php
										...
										for ($i=1; $i<=$achieve['i']; $i++):?>
											<div class="uap-admin-listing-ranks-achieve">
												<div><strong><?php echo esc_html($data['achieve_types'][$achieve['type_' . $i]]);?></strong></div>
