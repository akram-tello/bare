...
<div class="uap-wrapper">
	<div class="uap-page-title">Ultimate Affiliate Pro - <span class="second-text"><?php esc_html_e('Referrals (rewards)', 'uap');?></span></div>
		...
		<?php if (!empty($data['listing_items'])) : ?>
			<form  method="post" id="form_referrals">
				...
				<div class="uap-referrals-list-action-wrapper">
					<select name="list_action"><?php
						foreach ($data['actions'] as $k=>$v):
							?>
							<option value="<?php echo esc_attr($k);?>" <?php echo ($data['current_actions']==$k) ? 'selected' : '';?>><?php echo esc_html($v);?></option>
							<?php
						endforeach;
					?></select>
				...
				<div class="uap-referrals-list-numbers-wrapper">
					...
					<select name="uap_limit" class="uap-js-referral-list-limit-number" data-url="<?php echo esc_url($data['base_list_url'] . '&uap_limit=');?>" >
						<?php
					...
					<table class="wp-list-table widefat fixed tags uap-admin-tables">
						...
						<tbody class="ui-sortable uap-alternate">
							<?php foreach ($data['listing_items'] as $array) :  ?>
								<tr onmouseover="uapDhSelector('.hidden-div-referral-<?php echo esc_attr($array['id']);?>', 1);" onmouseout="uapDhSelector('.hidden-div-referral-<?php echo esc_attr($array['id']);?>', 0);">
									<th class="uap-vertical-align-top"><input type="checkbox" value="<?php echo esc_attr($array['id']);?>" name="referral_list[]" class="uap-delete-referral"/></th>
									<?php $temp_uid = $indeed_db->get_uid_by_affiliate_id($array['affiliate_id']);?>
									<td><a href="<?php echo admin_url('user-edit.php?user_id=' . $temp_uid);?>" target="_blank"><?php echo esc_html($array['affiliate_id']);?></a></td>
									<td><?php
									...
									?>
									<div id="referral_<?php echo esc_attr($array['id']);?>" class=" uap-visibility-hidden <?php echo esc_attr('hidden-div-referral-' . $array['id']);?>">
											<a href="<?php echo esc_url($data['url-add_edit'] . '&id=' . $array['id']);?>"><?php esc_html_e('Edit', 'uap');?></a>
											|
											<a onclick="uapDeleteFromTable(<?php echo esc_attr($array['id']);?>, 'Refferal', '#delete_referral_id', '#form_referrals');" href="javascript:return false;" class="uap-color-red"><?php esc_html_e('Delete', 'uap');?></a>
										</div>
									</td>
									<td><?php echo esc_html($array['id']);?></td>
									...
									<td>
										<?php
											...
											if (!empty($link)){
												echo esc_uap_content('<a href="' . $link . '" target="_blank">' . $array['reference'] . '</a>');
											} else {
												echo esc_html($array['reference']);
											}
										?>
									</td>
									<td><?php echo esc_uap_content($array['description']);?></td>
									<td><?php echo esc_uap_content('<b>' . uap_format_price_and_currency($array['currency'], $array['amount']) . '</b>');?></td>
												...
												foreach ($status_arr as $k=>$v){
													if ($k!=$array['status']){
													 if($i != 1){
													  ...
													  $i++;
													?>
													<span class="refferal-chang-status uap-js-referral-list-change-status" data-value="<?php echo esc_attr($array['id'] . '-' . $k);?>" ><?php esc_html_e('Mark as ', 'uap');?><?php echo esc_html($v);?></span>
													<?php
													}
												}
											?>
		...
		<table class="wp-list-table widefat fixed tags uap-admin-tables">
			...
			<tbody class="ui-sortable uap-alternate">
				<tr>
						<td colspan="2"><?php esc_html_e('No Referrals Stored.', 'uap');?></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
