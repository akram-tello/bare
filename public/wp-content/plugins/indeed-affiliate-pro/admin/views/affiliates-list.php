...
<div class="uap-wrapper uap-affiliate-list-wrapper">
		...
		?>
		<div class="uap-filters-wrapper <?php echo esc_attr($hidded); ?>" >
			<form method="get" action="<?php echo esc_attr($data['base_list_url']);?>">
		...
		<?php if ($data['listing_affiliates']):?>
			<div class="uap-number-to-display">
				...
				<select name="uap_limit" class="uap-js-affiliate-list-limit" data-url="<?php echo esc_url($data['base_list_url'].'&uap_limit=');?>">
					<?php
			...
			<form  method="post" id="form_affiliates">
				...
				<tbody class="ui-sortable">
					<?php $i = 1;
						...
						<tr onmouseover="uapDhSelector('#aff_<?php echo esc_attr($id);?>', 1);" onmouseout="uapDhSelector('#aff_<?php echo esc_attr($id);?>', 0);" class="<?php echo ($i%2==0) ? 'alternate' : '';?>">
							<th><input type="checkbox" value="<?php echo esc_attr($id);?>" name="affiliate_id_arr[]" class="uap-delete-affiliates"/></th>
							<td><?php echo esc_html($id);?></td>
                            <td class="uap-cel-top-psition">
							<?php if ( !empty( $data['rankings'] ) ):?>
											<?php if ( empty( $data['rankings'][$id] ) ):?>
                                            		<!--img src="<?php echo esc_url(UAP_URL);?>assets/images/uap_trophy.png"/>
													 <div class="uap-userprofile-top-position-table">#<?php echo ( isset( $data['rankings_last_place'] ) ) ? esc_html($data['rankings_last_place']) : esc_html('N/A');?></div-->
											<?php else :?>
													<img src="<?php echo UAP_URL;?>assets/images/uap_trophy.png" alt="<?php echo esc_attr($data['rankings'][$id]);?>"/>
                           							 <div class="uap-userprofile-top-position-table">#<?php echo esc_html($data['rankings'][$id]);?></div>
											<?php endif;?>
									<?php endif;?>

                             </td>
							<td class="uap-column-username">
                            <img src="<?php echo uap_get_avatar_for_uid($arr['uid']);?>" class="uap-admin-affiliate-list-avatar" alt="<?php echo esc_attr($arr['username']);?>"/>
                            <span class="uap-admin-affiliate-list-details">
															<span class="uap-list-affiliates-name-label"><?php echo esc_html($arr['name']);?></span>
															<span class="uap-admin-affiliate-list-details-unserame"><?php echo esc_uap_content($this->print_flag_for_affiliate($arr['uid']) . $arr['username']);?>
														</span>
								<?php
									if ($arr['payment_settings']):
										...
										switch ($arr['payment_settings']['type']):
											case 'paypal':
												...
												?>
												<a href="<?php echo esc_url($data['base_view_payment_settings_url'] . $arr['uid']);?> " target="_blank">
											...
											case 'bt':
												...
												?>
												<a href="<?php echo esc_url($data['base_view_payment_settings_url'] . $arr['uid']);?> " target="_blank">
													<span class="uap-admin-aff-payment-type <?php echo esc_attr($payment_class);?>"><?php esc_html_e('Bank Transfer', 'uap');?></span>
												</a>
												<?php
												break;
											case 'stripe':
											...
											case 'stripe_v2':
												...
												?>
												<a href="<?php echo esc_url($data['base_view_payment_settings_url'] . $arr['uid']);?> " target="_blank">
													<span class="uap-admin-aff-payment-type <?php echo esc_attr($payment_class);?>"><?php esc_html_e('Stripe V2', 'uap');?></span>
												</a>
												<?php
												break;
											case 'stripe_v3':
												...
												?>
												<a href="<?php echo esc_url($data['base_view_payment_settings_url'] . $arr['uid']);?> " target="_blank">
													<span class="uap-admin-aff-payment-type <?php echo esc_attr($payment_class);?>"><?php esc_html_e('Stripe V3', 'uap');?></span>
												</a>
												<?php
												break;
										endswitch;
								...
								<div id="aff_<?php echo esc_attr($id);?>" class="uap-visibility-hidden">
									<a href="<?php echo esc_url($data['url-add_edit'] . '&id=' . $arr['uid']);?>"><?php esc_html_e('Edit', 'uap');?></a>
                                    | <a onclick="uapDeleteFromTable(<?php echo esc_attr($id);?>, 'Affiliate', '#delete_affiliate', '#form_affiliates');" href="javascript:return false;" class="uap-color-red"><?php esc_html_e('Delete', 'uap');?></a>

                                    | <a  href="<?php echo esc_url($data['affiliate_profile_url'] . '&affiliate_id=' . $id);?>" target="_blank"><?php esc_html_e('Affiliate Profile', 'uap');?></a>
                                    | <a  href="<?php echo esc_url($data['base_transations_url'] . '&affiliate=' . $id);?>"><?php esc_html_e('Transactions', 'uap');?></a>
                                    | <a  href="<?php echo esc_url($data['base_reports_url'] . '&affiliate_id=' . $id);?>"><?php esc_html_e('Reports', 'uap');?></a>
									<?php if ($arr['role']=='pending_user'):?>
										| <a onClick="uapApproveAffiliate(<?php echo esc_attr($arr['uid']);?>);" href="javascript:return false;"><?php esc_html_e('Approve Affiliate', 'uap');?></a>
									<?php endif;?>
									<?php if ($arr['email_status']==-1): ?>
										<span id="<?php echo esc_attr('approve_email_' . $arr['uid']);?>" onClick="uapApproveEmail(<?php echo esc_attr($arr['uid']);?>, '<?php esc_html_e("Verified", "uap");?>');">
										| <span class="uap-special-action-link"><?php esc_html_e('Approve E-mail', 'uap');?></span>
										</span>
									<?php endif;?>
								</div>
							</td>
							<td><a href="mailto:<?php echo esc_attr($arr['email']);?>" target="_blank"><?php echo esc_html($arr['email']);?></a></td>
							<?php $style = (isset($arr['rank_color'])) ? 'uap-box-background-' . $arr['rank_color'] : 'uap-box-background-c9c9c9;';?>
							<td><div class="rank-type-list <?php echo esc_attr($style);?>"><?php echo esc_html($arr['rank_label']);?></div></td>

							<td class="uap-affiliate-list-counts">
								<div><?php echo isset($arr['stats']['visits']) ? esc_html($arr['stats']['visits']) : '';?></div>
								<?php if (!empty($arr['stats']['visits'])): ?>
									<a href="<?php echo esc_url($data['base_visits_url'] . '&affiliate_id=' . $id);?>"><?php esc_html_e('View', 'uap');?></a>
								<?php endif;?>
							</td>
                    ...
                    <td class="uap-affiliate-list-counts">
								...
								<?php if (!empty($data['base_referrals_url']) && $arr['stats']['referrals']): ?>
									<a href="<?php echo esc_url($data['base_referrals_url'] . '&affiliate_id=' . $id);?>"><?php esc_html_e('View', 'uap');?></a>
								<?php endif;?>
							</td>
							<td class="uap-affiliate-list-counts">
								...
								<?php if (!empty($arr['stats']['paid_payments_value'])): ?>
									<a href="<?php echo esc_url($data['base_paid_url'] . '&affiliate=' . $id);?>"><?php esc_html_e('View', 'uap');?></a>
								<?php endif;?>
							</td>
							<td class="uap-affiliate-list-counts">
								...
								<?php if (!empty($arr['stats']['unpaid_payments_value']) && (float)$arr['stats']['unpaid_payments_value'] > 0 ):?>
									<div><a href="<?php echo esc_url($data['base_unpaid_url'] . '&affiliate=' . $id);?>"><?php esc_html_e('Proceed', 'uap');?></a> | <a href="<?php echo esc_url($data['base_pay_now'] . '&affiliate=' . $id);?>"><?php esc_html_e('Pay All', 'uap');?></a></div>
								<?php endif;?>
							</td>
									...
									<?php if (!empty($data['show_ppc'])):?>
										<div>
											...
											<?php echo esc_html__('CPC: ', 'uap') . esc_uap_content($ppc);?>
										</div>
									<?php endif;?>
							...
							<td><div class="uap-subcr-type-list <?php echo esc_attr($pending);?>"><?php echo (isset($data['ranks_list'][$arr['role']])) ? $data['ranks_list'][$arr['role']] : '';?></div></td>
							<?php if (!empty($data['email_verification'])):?>
							<td><?php
                ...
                <div class="referral-status-verified uap-affiliate-profile-button"><a  href="<?php echo esc_url($data['affiliate_profile_url'] . '&affiliate_id=' . $id);?>"  target="_blank"><?php esc_html_e('Affiliate Profile', 'uap');?></a></div>
								<div class="referral-status-verified uap-affiliate-transactions-button"><a  href="<?php echo esc_url($data['base_transations_url'] . '&affiliate=' . $id);?>"><?php esc_html_e('Transactions', 'uap');?></a></div>
								<?php if (!empty($data['mlm_on']) && $indeed_db->affiliate_has_childrens($id) ) : ?>
									<div class="referral-status-unverified uap-mlm-button"><a  href="<?php echo esc_url($data['mlm_matrix_link'] . $arr['username']);?>"><?php esc_html_e('MLM Matrix', 'uap');?></a></div>
								<?php endif;?>
								<div class="referral-status-unverified uap-reports-button" ><a href="<?php echo esc_url($data['base_reports_url'] . '&affiliate_id=' . $id);?>"><?php esc_html_e('Reports', 'uap');?></a></div>
								<div class="uap_frw_button uap_small_grey_button uap-admin-do-send-email-via-ump" data-uid="<?php echo esc_attr($arr['uid']);?>"><?php esc_html_e('Direct Email', 'uap');?></div>
