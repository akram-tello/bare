	...
	class UapMainAdmin{
		...
		public function return_global_errors(){
			...
			if (current_user_can('manage_options')){
				echo esc_uap_content($this->print_unregistered_notice(TRUE));
			}
		}
		...
		private function print_top_messages(){
			 ...
			 * @return string
			 */
			echo esc_uap_content($this->print_unregistered_notice(FALSE));
		...
		private function print_ranks(){
			...
			if ($data['subtab']=='list'){
				if (!empty($_POST['save']) && !empty($_POST['uap_admin_forms_nonce']) && wp_verify_nonce( sanitize_text_field($_POST['uap_admin_forms_nonce']), 'uap_admin_forms_nonce' ) ){
					$indeed_db->rank_save_update( uap_sanitize_array($_POST) );
		...
		private function print_reports(){
			...
			if ($subtab=='reports'){
				...
				$data['selected'] = (isset($_POST['search'])) ? uap_sanitize_array($_POST['search']) : 'all_time';
		...
		private function cpm_commission()
		{
				...
				$this->print_top_messages();
				if (!empty($_POST['save'])){
					$indeed_db->save_settings_wp_option('cpm_commission', sanitize_textarea_field($_POST) );
					if (isset($_POST['cpm_commission_value'])){
						foreach ($_POST['cpm_commission_value'] as $id=>$value){
							$indeed_db->update_rank_column_force_empty('cpm_commission', sanitize_text_field($id), uap_sanitize_array($value) );
						}
					}
				}
		...
		public function add_style_scripts(){
			...
			wp_enqueue_style('uap_main_admin_style', UAP_URL . 'assets/css/main_admin.css', array(), 8.3 );
			if ($is_plugin_page){
				...
				wp_enqueue_style('uap_main_public_style', UAP_URL . 'assets/css/main_public.min.css', array(), 8.3 );
				wp_enqueue_style('uap_templates', UAP_URL . 'assets/css/templates.min.css', array(), 8.3 );
			}
			...
			wp_enqueue_media();
			wp_register_script('uap_admin_js', UAP_URL . 'assets/js/admin-functions.min.js', ['jquery'], 8.3 );
			...
			if ( $pagenow == 'plugins.php' ){
					...
					wp_enqueue_script( 'indeed_sweetalert_js', UAP_URL . 'assets/js/sweetalert.js', ['jquery'], 8.3 );
			...
			if ($is_plugin_page){
				...
				wp_enqueue_script('uap-jquery.flot.js', UAP_URL . 'assets/js/jquery.flot.js', array( 'jquery' ), 8.3 );
				wp_enqueue_script('uap-bootstrap.js', UAP_URL . 'assets/js/bootstrap.js', array('jquery'), 8.3 );
				wp_enqueue_script('uap-jquery.flot.pie.js', UAP_URL . 'assets/js/jquery.flot.pie.js', array('jquery'), 8.3 );
				wp_enqueue_script('uap-jquery_form_module', UAP_URL . 'assets/js/jquery.form.js', array('jquery'), 8.3 );
				wp_enqueue_script('uap-jquery.uploadfile', UAP_URL . 'assets/js/jquery.uploadfile.min.js', array('jquery'), 8.3 );
				wp_enqueue_script('uap-jquery.flot.time.js', UAP_URL . 'assets/js/jquery.flot.time.js', array('jquery'), 8.3 );
				wp_register_script('uap_public', UAP_URL . 'assets/js/public-functions.min.js', ['jquery'], 8.3 );
				...
				wp_enqueue_script( 'uap-select2', UAP_URL . 'assets/js/select2.min.js', array(), 8.3 );
